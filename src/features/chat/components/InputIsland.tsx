...
export const InputIsland = () => {
    ...
    const connLabel = connectionState === 'connecting' ? 'Connecting to server...'
        : connectionState === 'disconnected' ? 'Reconnecting...'
            : connectionState === 'kicked' ? 'Disconnected (Opened in another tab)'
                : null;
    ...
    return (
        <div className="input-island-container">
            ...
            <div className="emoji-popover" data-show={isEmojiOpen}>
                {EMOJIS.map(emoji => (
                    <span key={emoji} className="emoji-item" onClick={() => addEmoji(emoji)}>{emoji}</span>
                ))}
            </div>
            ...
            >
                ...
                <button className="tool-btn" disabled={isDisabled} onClick={() => fileInputRef.current?.click()} title="Attach file">
                    <Plus size={18} />
                </button>
                <button
                    className="tool-btn"
                    disabled={isDisabled}
                    onClick={(e) => { e.stopPropagation(); toggleEmoji(); }}
                    title="Emoji"
                >
                    <Smile size={18} />
                </button>
