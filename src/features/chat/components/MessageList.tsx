...
const StatusIcon = memo(({ status }: { status?: Message['status'] }) => {
  if (!status || status === 'sending')
    return (
      <span title="Sending">
        <Clock size={11} className="msg-status-icon" />
      </span>
    )
  if (status === 'sent')
    return (
      <span title="Sent">
        <Check size={11} className="msg-status-icon" />
      </span>
    )
  if (status === 'delivered')
    return (
      <span title="Delivered">
        <CheckCheck size={11} className="msg-status-icon" />
      </span>
    )
  if (status === 'read')
    return (
      <span title="Read">
        <CheckCheck size={11} className="msg-status-icon msg-status-read" />
      </span>
    )
  return null
})
...
export const MessageList = ({
  ...
  registerScroll: (fn: (id: string) => void) => void
}) => {
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; msg: Message } | null>(
    null
  )
  ...
  const handleCopy = useCallback((msg: Message) => {
    if (msg.content)
      navigator.clipboard.writeText(msg.content.replace(/<[^>]+>/g, '')).catch(() => {})
    setContextMenu(null)
  }, [])
          ...
          <div className="context-reactions">
            {QUICK_REACTIONS.map(emoji => (
              <button
                ...
                onClick={() => handleReact(contextMenu.msg, emoji)}
              >
                {emoji}
              </button>
            ))}
