...
export const useChatStore = create<ChatState>((set) => ({
    ...
    updateMessageStatus: (id, status) => set((state) => {
        ...
        const rank: Partial<Record<string, number>> = {
            sending: 0, sent: 1, delivered: 2, read: 3, retracted: 4,
        };
    ...
    retractMessage: (id) => set((state) => {
        const messages = state.messages.map((m): Message => m.id === id
            ? { ...m, status: 'retracted', content: '', isFile: false, isImage: false, replyTo: undefined }
            : m);
        saveMessages(messages);
    ...
    addReaction: (msgId, emoji, mine) => set((state) => {
        const messages = state.messages.map(m => {
            ...
            if (existing) {
                if (mine && existing.myReaction) {
                    ...
                    reactions[emoji] = { count: existing.count + 1, myReaction: mine ? true : existing.myReaction };
                }
            } else {
    ...
    setPeerTyping: (isTyping, name) => set((state) => {
