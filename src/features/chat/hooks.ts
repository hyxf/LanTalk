...
export const useMentionAutocomplete = (inputValue: string) => {
    ...
    const suggestions = query !== null
        ? users
            .filter(u => u.id !== myId && u.name.toLowerCase().startsWith(query))
            .slice(0, 5)
        : [];
...
export const useMessageInput = () => {
    const { inputValue, setInputValue, addMessage, toggleEmoji, isEmojiOpen,
        updateMessageStatus, connectionState, replyTo, setReplyTo } = useChatStore();
    ...
    const sendMessage = () => {
        ...
        const replyToData = replyTo && replyTo.type !== 'system'
            ? { id: replyTo.id, content: replyTo.content, senderName: replyTo.senderName, type: replyTo.type as 'own' | 'other' }
            : undefined;

        addMessage({ content: inputValue, type: 'own', time, status: 'sending', id: msgId, replyTo: replyToData });
    ...
    const sendFile = (file: File) => {
        ...
        reader.onload = (ev) => {
            ...
            if (isImage) {
                ...
                `;
                addMessage({ content: htmlContent, type: 'own', time, isFile: true, isImage: true, status: 'sending', id: msgId });
            } else {
                const htmlContent = `
                    <div class="file-card">
                        ${FILE_ICON_SVG}
                ...
                `;
                addMessage({ content: htmlContent, type: 'own', time, isFile: true, status: 'sending', id: msgId });
            }

            socketClient.send({ type: 'file', msgId, fileName: file.name, fileSize: file.size, fileData, mimeType, time });
            setTimeout(() => updateMessageStatus(msgId, 'sent'), 100);
        };
    ...
    return {
        inputValue, isEmojiOpen, textAreaRef, fileInputRef, connectionState,
        replyTo, setReplyTo, showMentions,
        handleInput, handleKeyDown, sendMessage, toggleEmoji,
        handleFileSelect, sendFile, addEmoji, insertMention,
    };
};
