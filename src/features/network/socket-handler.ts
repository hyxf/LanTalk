    ...
    useEffect(() => {
        ...
        const unsubscribe = socketClient.onMessage((msg) => {
            switch (msg.type) {
                ...
                case 'userList': {
                    setUsers(msg.users.map(u => ({
                        id: u.id, initials: u.initials, name: u.name,
                        status: u.status, lastMessage: '', isActive: false,
                    })));
                    break;
                }
                case 'message': {
                    ...
                    const mentioned = !!myCurrentName &&
                        msg.content.toLowerCase().includes(`@${myCurrentName.toLowerCase()}`);
                    receiveMessage({
                        content: msg.content, type: 'other', time: msg.time,
                        senderName: msg.senderName, status: 'read',
                        id: msg.msgId,
                        replyTo: msg.replyTo,
                        mentioned,
                    });
                ...
                case 'file': {
                    ...
                    receiveMessage({
                        content: htmlContent, type: 'other', time: msg.time,
                        isFile: true, isImage, senderName: msg.senderName, status: 'read',
                        id: msg.msgId,
                    });
                    playPingSound();
